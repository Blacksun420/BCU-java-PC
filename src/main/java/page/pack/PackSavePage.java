...
public class PackSavePage extends DefaultPage {
    ...
    private void setMap(StageMap smap, boolean nmod) {
        ...
        if (smap != null) {
            ...
            recRemoval(smap, pm);
            LinkedList<StageMap> reqCol = new LinkedList<>();
            reqCol.add(smap);
            for (StageMap sm : pm)
                if (!sm.unlockReq.isEmpty() && containsAny(reqCol, sm))
                    reqCol.add(sm);
            pm.removeAll(reqCol);
    ...
    private boolean containsAny(LinkedList<StageMap> reqCol, StageMap sm) {
        for (StageMap sp : sm.unlockReq)
            if (reqCol.contains(sp))
                return true;
        return false;
    }
